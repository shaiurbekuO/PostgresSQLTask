...
select id, brand from cars where brand in ('Hyundai');
-- #4 Бардык кызыл жана кок машиналарды чыгарыныз
select id, brand, color from cars where color in('Red', 'Blue');
-- #5 Бардык 2000-жылдан 2010-жылга чейинки чыгарылган машиналарды чыгарыныз
select brand, year_of_issue from cars where year_of_issue>2000 and year_of_issue<2010;
-- #6 Chevrolet брендинде канча машина бар экенин чыгарыныз
select count(*), brand from cars where brand in ('Chevrolet') group by brand ;
-- #7 Машиналардын чыгарылган жылдарынын орточо маанисин чыгарыныз
select round(avg(year_of_issue)) as average_year_of_manufacture from cars;
-- #8 Audi, Toyota, Kia жана Ford брендиндеги машиналдарды чыгарыныз
select id, brand from cars where brand in(' Audi', 'Toyota', 'Kia', 'Ford');
-- #9 Модели Т менен башталган машиналарды чыгарыныз
select id, brand from cars where  brand like 'T%';
-- #10 Модели “е” менен буткон машиналарды чыгарыныз
select id, brand from cars where  brand like '%e';
-- #11 Бренди 5 символдон турган машиналарды чыгарыныз
select id, brand from cars where  brand like '_____';
...
select  max(price), min(price) as price from cars;
-- #14 TOYOTA дан башка машиналарды чыгарыныз
select distinct brand from cars where brand <> 'Toyota';
-- #15 Эн кымбат 10 машинаны чыгарыныз
select id, price from cars order by price desc fetch first 10 rows only ;
select id, price from cars order by price desc limit 10;
-- #16 Эн жаны машиналардыдын 5 тен 15 ке чейинкисин чыгарыныз
select id, year_of_issue from cars order by year_of_issue desc limit 15 offset 5;
-- #17 1995 - жылдан 2005- ке чейинки машиналарадан башкаларын чыгарыныз
select id, year_of_issue from cars where  year_of_issue not between 1995 and 2005;
select id, year_of_issue from cars where year_of_issue< 1995 or year_of_issue> 2005;
-- #18 Бир тустогу саны эн коп болгон машиналарды чыгаргыла
select color, count(*) as number_of_cars from cars
group by color order by number_of_cars desc limit 1;